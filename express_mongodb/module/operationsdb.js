...
export const buscar3 = async (c, f) => {
    ...
    try {
        ...
        console.log('Documento agregado correctamente:', resultado._id);
        return resultado;
...
export const agregarNO = async (c, f) => {
    const url = 'mongodb://localhost:27017/';
    const client = await MongoClient.connect(url);
    try {
        const collection = client.db(db).collection(c);
        const resultado = await collection.insertOne(f);
        console.log(resultado);
        return resultado;
...
export const agregar = async (dbNombre, coleccionNombre, documento) => {
    const url = 'mongodb://localhost:27017/';
    const client = await MongoClient.connect(url);
